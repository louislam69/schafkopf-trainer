...
import { runBidding, applyGame, mulberry32, seatOrder, bidFor, playFor, kontraFor, reizung } from "./table.js";
...
function finishBidding() {
  ...
  /* Mit der Reizung, damit Bots und Nachbesprechung beim Auswürfeln
     keine Blätter annehmen, mit denen am Tisch anders gereizt worden
     wäre. Sitz 0 ist der Mensch und bleibt ungeprüft. */
  app.st = applyGame(app.st, b.best.bid, b.best.p, reizung(b.bids, (p) => (p === 0 ? null : levelOf(p))));
...
function finishHand() {
  ...
  modal(`
    ...
    <p class="muted" id="auswertungszeile">Die Auswertung läuft …</p>
    <div class="row">
  ...
  render();
  /* Die Nachbesprechung würfelt je Entscheidung 100 Welten aus — am
     Rechner rund 0,3 s je Blatt, in schlechten Stellungen über eine
     Sekunde, auf dem Handy mehr. Erst das Ergebnis zeigen, dann rechnen:
     sonst steht die Seite still, bevor man überhaupt weiß, wie es
     ausging. Die 50 ms geben dem Browser Zeit zum Malen. */
  app.offeneAuswertung = app.decisions;
  setTimeout(auswertungFertig, 50);
}

/* Rechnet die ausstehende Nachbesprechung — aus dem Takt nach dem
   Ergebnisfenster oder sofort, wenn jemand schneller auf „Auswertung
   ansehen" drückt. Wer zuerst kommt, rechnet; der andere findet sie
   fertig vor. */
function auswertungFertig() {
  const entscheidungen = app.offeneAuswertung;
  if (!entscheidungen) return;
  app.offeneAuswertung = null;
  app.review = buildReview(entscheidungen, rng);
  store.stats.decisions += app.review.items.length;
  store.stats.loss += app.review.items.reduce((a, i) => a + i.augen, 0);
  save(store);
  const zeile = $("#auswertungszeile");
  if (zeile) {
    const n = app.review.mistakes.length, h = app.review.hinweise.length;
    const hin = h ? `${h === 1 ? "ein Buch-Hinweis" : `${h} Buch-Hinweise`} ohne Einfluss aufs Ergebnis` : "";
    zeile.textContent = n
      ? `${n === 1 ? "Eine Stelle" : `${n} Stellen`} zum Nachschauen — im Schnitt ${komma(app.review.avgLoss)} Prozentpunkte Gewinnchance pro Zug liegen gelassen.${h ? ` Dazu ${hin}.` : ""}`
      : h ? `Nichts, was Gewinnchance gekostet hat — ${hin} zum Nachlesen.`
        : "Sauber gespielt, nichts Auffälliges gefunden.";
  }
  render();
}
...
/* Gewinnchance statt Augen: „−12 %" heißt, der Zug hat zwölf
   Prozentpunkte der Verteilungen verschenkt, in denen die eigene
   Partei ihr Ziel erreicht. */
/* Ein Merksatz passt, aber der Zug hat messbar nichts gekostet. Der
   Hinweis bleibt — als Empfehlung des Buches, nicht als Fehler. */
const NEUTRAL = "Anders zu spielen hätte hier am Ausgang kaum etwas geändert. Das Buch empfiehlt trotzdem:";

const komma = (x) => x.toLocaleString("de-DE", { minimumFractionDigits: 1, maximumFractionDigits: 1 });
const verlustText = (loss) => (loss >= 0.5 ? `−${Math.round(loss)} %` : "±0 %");

/* Die Zahlen hinter dem Urteil, dazu — getrennt davon — Glück oder
   Pech mit den echten Karten. Das ± sind zwei Standardfehler. */
function chanceHtml(m) {
  const pct = (x) => Math.round(100 * x) + " %";
  /* „Besser" heißt nur, was die Welten auch hergeben. Liegt eine andere
     Karte bloß innerhalb der Streuung vorn, war der Zug keiner, den man
     bereuen müsste — dann steht das so da. */
  const zahlen = m.fehler
    ? `${cardName(m.best)} reicht in ${pct(m.anteilBest)} der möglichen Verteilungen, ${cardName(m.card)} in ${pct(m.anteil)} (± ${Math.round(m.genauigkeit)}).`
    : m.best !== m.card
      ? `${cardName(m.card)} reicht in ${pct(m.anteil)} der möglichen Verteilungen — kein messbarer Unterschied zur besten Karte (${cardName(m.best)}, ${pct(m.anteilBest)}, ± ${Math.round(m.genauigkeit)}).`
      : `Deine Karte reicht in ${pct(m.anteil)} der möglichen Verteilungen — besser ging es nicht.`;
  return `<p class="chance">${zahlen}</p>${m.glueck ? `<p class="glueck ${m.glueck.art}">${m.glueck.text}</p>` : ""}`;
}
...
function renderReview() {
  ...
  if (!app.review) {
    box.innerHTML = `<div class="empty"><h2>Noch nichts auszuwerten</h2><p>Spiel ein Blatt zu Ende — danach steht hier jeder Zug, der Gewinnchance gekostet hat, mit dem passenden Prinzip.</p></div>`;
    return;
  }
  const r = app.review;
  /* Erst die Fehler, dann die Hinweise in Stichreihenfolge. Ohne beides
     wie bisher die ersten drei Züge, damit die Seite nicht leer ist. */
  const hinweise = r.hinweise.slice().sort((a, b) => a.trick - b.trick);
  const list = r.mistakes.length || hinweise.length ? r.mistakes.concat(hinweise) : r.items.slice(0, 3);
  box.innerHTML = `
    <h2>Auswertung</h2>
    <p class="muted">${r.items.length} echte Entscheidungen, im Schnitt ${komma(r.avgLoss)} Prozentpunkte Gewinnchance unter dem besten Zug. Beurteilt über die Verteilungen, die zu deinem Wissen passten — nicht über die echten Karten.</p>
    ${list.map((m, i) => `
      <article class="lesson${m.hinweis ? " hinweis" : ""}">
        <header>
          <span class="stich">${m.trick}. Stich</span>
          ${m.hinweis
            ? `<span class="verlust hinweis">Buch-Hinweis</span>`
            : `<span class="verlust ${m.loss >= 15 ? "hoch" : m.loss >= 5 ? "mittel" : "klein"}">${verlustText(m.loss)}</span>`}
        </header>
        <div class="lesson-cards">
          <div><small>gespielt</small>${cardHtml(m.card, { small: true })}</div>
          ${m.fehler ? `<div><small>besser</small>${cardHtml(m.best, { small: true })}</div>` : ""}
        </div>
        ${chanceHtml(m)}
        ${m.hinweis ? `<p class="neutral">${NEUTRAL}</p>` : ""}
...
function renderDrillPanel() {
  ...
  if (!app.drill) {
    box.innerHTML = `
      ...
      ${store.drills.length ? `<ul class="drilllist">${store.drills.slice(0, 8).map((d) => `<li>${gameLabelSafe(d.state.game)} · ${d.einheit === "prozent" ? verlustText(d.verlust) : `${d.verlust} Augen`} · gespielt ${cardName(d.gespielt)}</li>`).join("")}</ul>` : ""}`;
...
function answerDrill(card) {
  ...
  d.answer = card;
  d.feedback = `
    <div class="verdict ${v.ok ? "gut" : "schlecht"}">${v.ok ? "Passt." : `Kostet ${Math.round(v.loss)} Prozentpunkte Gewinnchance.`}</div>
    ${v.ok ? "" : `<p>Besser wäre ${v.bestText} gewesen.</p>`}
    ${v.glueck ? `<p class="glueck ${v.glueck.art}">${v.glueck.text}</p>` : ""}
    ${v.ok && v.notes.length ? `<p class="neutral">${NEUTRAL}</p>` : ""}
    ...
    <p class="muted small">Bewertung: ${v.ranking.slice(0, 3).map((e) => `${cardName(e.card)} ${Math.round(100 * e.anteil)} %`).join(" · ")} (Anteil der möglichen Verteilungen, in denen deine Partei ihr Ziel erreicht; ± ${Math.round(v.genauigkeit)})</p>`;
  renderDrillPanel();
}
