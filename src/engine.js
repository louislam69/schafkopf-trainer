...
function determinize(st, p, rng) {
  ...
  /* Gerufen werden darf nur eine Farbe, von der man selbst eine Karte
     hält (`sauspielOptions`). Solange der Rufer keine davon gespielt
     hat, steckt also mindestens eine noch in seinem Blatt. Ohne diese
     Probe gab jede fünfte Welt aus Sicht eines Gegenspielers dem Rufer
     keine — eine Verteilung, die es nicht geben kann. „Mindestens eine"
     lässt sich Karte für Karte nicht verbieten, deshalb wird die fertige
     Welt geprüft und notfalls neu gewürfelt. */
  const ruf = (c) => !isTrump(c, g) && c[0] === g.suit;
  const ruferHaeltRuffarbe = called && st.declarer !== p &&
    !st.tricks.some((t) => t.plays.some((x) => x.p === st.declarer && ruf(x.card))) &&
    !st.trick.some((x) => x.p === st.declarer && ruf(x.card));
  /* Die Reizung als weiche Probe, wie in GIB: eine Welt, in der ein Bot
     mit seinem Blatt anders gereizt hätte, als er es am Tisch tat, gibt
     es nicht. Gegen Bots ist das exakt — ihre Ansage ist Code. Weich
     heißt: findet sich keine passende Welt, gilt die erste, die
     wenigstens die harten Regeln erfüllt. */
  const mitReizung = !!(st.reizung && ansagePruefer);
  /* Scheitert die Probe einmal ganz, scheitert sie in derselben
     Stellung meist wieder — und jeder Versuch kostet rund 0,1 ms. Wer
     schon einmal ausweichen musste, bekommt deshalb nur die kurze
     Runde. Ohne diese Bremse fraßen 3 % der Welten 44 % der Zeit. */
  const versuche = mitReizung && !reizungScheitert.has(st) ? 100 : 40;
  let ersatz = null;
  determinizeZaehler.welten++;
  for (let attempt = 0; attempt < versuche; attempt++) {
    if (!ersatz && attempt >= 40) break;
    ...
    if (ok && ruferHaeltRuffarbe && !out[st.declarer].some(ruf)) ok = false;
    if (ok) {
      const hands = st.hands.slice();
      others.forEach((q) => (hands[q] = out[q]));
      const welt = withSampledPartner({ ...st, hands });
      if (!mitReizung || ansagePruefer(welt, p)) return welt;
      if (!ersatz) ersatz = welt;
    }
  }
  if (ersatz) { determinizeZaehler.ohneReizung++; reizungScheitert.add(st); return ersatz; }
  determinizeZaehler.rueckfall++;
  const deck = pool.slice();
  const hands = st.hands.slice();
...
/* Die Ansage der Bots steht in table.js, und table.js importiert die
   Engine — umgekehrt ginge es nur im Kreis. Deshalb meldet table.js
   seine Probe hier an; ohne sie (etwa in Tests, die nur die Engine
   laden) bleibt die Reizung unberücksichtigt. */
let ansagePruefer = null;
function setzeAnsagePruefer(f) { ansagePruefer = f; }
const reizungScheitert = new WeakSet();

/** Wie oft `determinize` ausweichen musste — zum Messen und für die Tests. */
const determinizeZaehler = { welten: 0, ohneReizung: 0, rueckfall: 0 };
...
/* Jede Karte auf dem *echten* Blatt zu Ende gespielt, in Augen — also
   mit dem Wissen hinterher. Die Nachbesprechung urteilt damit nicht
   mehr (siehe `infoWerte` in pimc.js); übrig sind `npm run quality`
   und `npm run buch`, die ihn als groben, schnellen Maßstab behalten. */
